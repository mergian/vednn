...
//  const int64_t dilationHeight = pParamConv->dilationHeight;		// must be 1
...

  {
    ...
    __vr vrhw = _vel_vaddul_vvvl(vrx, _vel_vmulul_vsvl(inWidth,vry, nY*outWidth), nY*outWidth) ;
	...
	if ( (outChannelGroup & 0x01) == 1 ) {
	  ...
	  for (int64_t y=0; y<outHeight; y+=nY) {
		...
		for (int64_t s = 0; s < kernWidth; s++) {
		  __vr vrpin = _vel_vsfa_vvssl(vrhw, 2, (uint64_t)(pInChannel+r*inWidth+s), vl) ;
