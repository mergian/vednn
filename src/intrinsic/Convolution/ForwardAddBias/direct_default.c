...

  {
	...
	if ( (outChannelGroup & 0x01) == 1 ) {
	  ...
	  for (int64_t op = 0; op < oPixels; op+=VLEN) {
	    ...
	    __vr vri   = _vel_vaddsl_vsvl(-padHeight, _vel_vmulsl_vsvl(strideHeight, vry, vl), vl) ;
	    __vr vrj   = _vel_vaddsl_vsvl(-padWidth,  _vel_vmulsl_vsvl(strideWidth, vrx, vl), vl) ;

	    for (int64_t r = 0; r < kernHeight; r++) {
	      for (int64_t s = 0; s < kernWidth; s++) {
		__vr vrh = _vel_vaddsl_vsvl(r*dilationHeight, vri, vl) ;
